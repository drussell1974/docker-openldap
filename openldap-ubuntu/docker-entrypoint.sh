...
_ldapadd_init_ldif() {
	...
	# 4. replace IP Address of NFS Server in automountInformation

	# 5. add ldap directory - command should be format e.g. ldapadd -w <PASSWORD> -D "cn=admin,dc=example,dc=net" -f init/init.ldif
	...
	if test -f ${LDAP_INIT_DIR}/${LDAP_INIT_FILE}; then
		...
		if [ ${LDAP_BASE_DN:-$LDAP_BASE_DN} -a ${LDAP_ADMIN_USER:-$LDAP_ADMIN_USER} -a ${LDAP_ADMIN_PASSWORD:-$LDAP_ADMIN_PASSWORD} ]; then 
			...
			sed -i "s/automountInformation: 0.0.0.0/automountInformation: ${LDAP_NFS_SERVER_IPADDR}/" ${LDAP_INIT_DIR}/${LDAP_INIT_FILE}
